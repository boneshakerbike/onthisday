...
All endpoints protected by NextAuth middleware except `/api/auth/*` and `/api/health`.
...
| `/api/health` | GET | Database health check and duplicate detection |
...
**Protected Routes:** All except `/login`, `/api/auth/*`, `/api/health`
...
3. Returns HTML story + token usage metrics

## Important Files

| File | Lines | Purpose |
|------|-------|---------|
| `src/app/page.tsx` | ~636 | Main dashboard with all UI logic |
| `src/lib/db.ts` | ~360 | Database abstraction layer |
| `src/app/api/generate/route.ts` | ~154 | Claude AI integration |
