...
// ============================================================================
// Usage Logs (Anthropic API Token Tracking)
// ============================================================================
...
async function init_usage_schema(): Promise<void> {
  const db = get_client();
  ...
  await db.execute(`CREATE INDEX IF NOT EXISTS idx_usage_logs_agent_id ON usage_logs(agent_id)`);
  await db.execute(`CREATE INDEX IF NOT EXISTS idx_usage_logs_model ON usage_logs(model)`);
  ...
  // Seed pricing data (current Anthropic rates)
  const pricing_seed = [
    ['claude-opus-4-5-20251101', 5.0, 25.0, 0.5, 6.25, '2025-11-01'],
    ['claude-sonnet-4-20250514', 3.0, 15.0, 0.3, 3.75, '2025-05-14'],
    ['claude-haiku-4-5-20251001', 1.0, 5.0, 0.1, 1.25, '2025-10-01'],
  ];

  for (const [model, input, output, cache_read, cache_create, date] of pricing_seed) {
    await db.execute({
      sql: `INSERT OR IGNORE INTO pricing_config (model, input_cost_per_mtok, output_cost_per_mtok, cache_read_cost_per_mtok, cache_creation_cost_per_mtok, effective_date)
            VALUES (?, ?, ?, ?, ?, ?)`,
      args: [model, input, output, cache_read, cache_create, date]
    });
  }
}

let usage_schema_initialized = false;
async function ensure_usage_schema(): Promise<void> {
  if (!usage_schema_initialized) {
    await init_usage_schema();
    usage_schema_initialized = true;
  }
}

export async function log_usage(entry: {
  id: string;
  provider_id: string | null;
  agent_id: string | null;
  route: string;
  ...
  await db.execute({
    sql: `INSERT INTO usage_logs (id, provider_id, agent_id, route, model, input_tokens, output_tokens, cache_read_input_tokens, cache_creation_input_tokens, status, retries, error_message)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    args: [
      entry.id, entry.provider_id, entry.agent_id, entry.route, entry.model,
      entry.input_tokens, entry.output_tokens,
      entry.cache_read_input_tokens, entry.cache_creation_input_tokens,
      entry.status, entry.retries, entry.error_message
    ]
  });
}

export async function get_usage_logs(opts: {
  start?: string;
  end?: string;
  model?: string;
  agent_id?: string;
  ...
  if (opts.start) { conditions.push('timestamp >= ?'); args.push(opts.start); }
  if (opts.end) { conditions.push('timestamp < ?'); args.push(opts.end); }
  if (opts.model) { conditions.push('model = ?'); args.push(opts.model); }
  if (opts.agent_id) { conditions.push('agent_id = ?'); args.push(opts.agent_id); }
  ...
  return result.rows.map(row => ({
    ...
    status: row.status as UsageLog['status'],
    retries: row.retries as number,
    error_message: (row.error_message as string) || null,
  }));
}

export async function get_usage_summary(opts: {
  start: string;
  end: string;
  ...
  const totals = await db.execute({
    sql: `SELECT
      ...
      COALESCE(SUM(CASE WHEN status = 'failure' THEN 1 ELSE 0 END), 0) as failures,
      COALESCE(SUM(CASE WHEN status = 'partial' THEN 1 ELSE 0 END), 0) as partials
    FROM usage_logs WHERE timestamp >= ? AND timestamp < ?`,
    args: [opts.start, opts.end]
  });

  const by_model = await db.execute({
    sql: `SELECT model,
      COUNT(*) as calls,
  ...
  const by_route = await db.execute({
    sql: `SELECT route,
      COUNT(*) as calls,
      COALESCE(SUM(input_tokens), 0) as input_tokens,
      COALESCE(SUM(output_tokens), 0) as output_tokens
  ...
  return {
    total_calls: t.total_calls as number,
    total_input_tokens: t.total_input as number,
    total_output_tokens: t.total_output as number,
    total_cache_read_tokens: t.total_cache_read as number,
    total_cache_creation_tokens: t.total_cache_creation as number,
    failures: t.failures as number,
    partials: t.partials as number,
    by_model: by_model.rows.map(row => ({
      model: row.model as string,
      calls: row.calls as number,
      input_tokens: row.input_tokens as number,
      output_tokens: row.output_tokens as number,
      cache_read_input_tokens: row.cache_read_input_tokens as number,
      cache_creation_input_tokens: row.cache_creation_input_tokens as number,
    })),
    by_route: by_route.rows.map(row => ({
      route: row.route as string,
      calls: row.calls as number,
      input_tokens: row.input_tokens as number,
      output_tokens: row.output_tokens as number,
...
export async function get_pricing_config(): Promise<PricingEntry[]> {
  await ensure_usage_schema();
  const db = get_client();

  const result = await db.execute('SELECT * FROM pricing_config ORDER BY model');
  return result.rows.map(row => ({
    model: row.model as string,
    input_cost_per_mtok: row.input_cost_per_mtok as number,
    output_cost_per_mtok: row.output_cost_per_mtok as number,
    cache_read_cost_per_mtok: row.cache_read_cost_per_mtok as number,
    cache_creation_cost_per_mtok: row.cache_creation_cost_per_mtok as number,
